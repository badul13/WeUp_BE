...
## 🎯 프로젝트 개요
**WeUp**은 대학생들의 효율적인 팀 프로젝트 수행을 돕기 위해 기획된 올인원(All-in-One) 협업 플랫폼입니다. 

단순한 일정/할 일(Todo) 관리를 넘어, **WebRTC 기반의 실시간 화상 회의**, **Redis 최적화 기반의 실시간 채팅**, 그리고 플라스크(Flask) 서버와 연동된 **AI 비서 기능**을 제공하여 비대면 상황에서도 압도적인 팀워크와 생산성을 지원합니다.
...
*   **해결:** 최신 채팅 내역은 Redis의 Sorted Set(`ZSet`)에 타임스탬프(`sentAt`)를 Score로 하여 캐싱했습니다. 채팅방 입장 시, 과거 데이터는 MySQL에서 가져오고 최신 데이터는 Redis `ZSet`의 `rangeByScore`로 가져와 메모리 단에서 O(log(N))의 속도로 빠르게 병합(Merge)하여 응답 속도를 획기적으로 개선했습니다.
...
*   **해결:** 낙관적 락 대신, 실시간 편집 상태를 브로드캐스팅하기 좋은 Redis의 `setIfAbsent`(SETNX)를 활용하여 분산 락(Lock)을 구현했습니다. 특정 사용자가 수정을 시작하면 TTL 동안 락을 점유(`lock:project:{id}`)하고, STOMP를 통해 다른 유저들에게 `EDIT_LOCK` 이벤트를 발행하여 UI 단에서 수정을 원천 차단하는 UX/기능을 완성했습니다.
...
*   **해결:** `User` 및 `Member` 엔티티에 `is_user_withdrawal`, `deleted_at` 등의 필드를 두어 논리적 삭제(Soft Delete) 방식을 도입했습니다. 이를 통해 과거의 데이터 흐름을 안전하게 보존하면서도 복구(`restore`) 기능을 유연하게 제공할 수 있는 실무적인 아키텍처를 구현했습니다.
...
## 🤝 팀원 개발 부분 및 협업 (본인 미구현)
   ...
   * WebSocket 세션 관리: STOMP 채널 인터셉터(Channel Interceptor) 구현 및 Redis를 활용한 실시간 소켓 접속자(Session) 상태 관리.
